...
import vg.skye.silicate.mixin.BlockEntityAkashicBookshelfMixin
import vg.skye.silicate.mixin.BlockEntitySlateMixin
...
object PatternTextureManager {
    ...
    private fun renderPattern(
        ...
        ps.push()
        val last = ps.peek()
        val mat: Matrix4f = last.model
        val normal: Matrix3f = last.normal
    ...
    private fun vertex(
        ...
        verts.vertex(mat, x, y, z)
            .color(-0x1)
            .uv(u, v).overlay(OverlayTexture.DEFAULT_UV).light(light)
    ...
    private fun createTexture(
        ...
        for (y in 0 until img.height) for (x in 0 until img.width) nativeImage.setPixelColor(x, y, img.getRGB(x, y))
